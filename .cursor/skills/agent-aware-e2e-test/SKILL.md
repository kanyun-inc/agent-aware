---
name: agent-aware-e2e-test
description: E2E testing workflow for agent-aware project. Agent generates a random buggy React app, then uses the main agent-aware skill for real-time monitoring and auto-fix. This also validates the main skill's effectiveness.
---

# Agent-aware E2E 测试 Skill

本 Skill 用于测试 agent-aware 的完整功能，同时**验证主 Skill 的可行性**。

---

## 与主 Skill 的关系

本 Skill 流程：
```
Phase 1: 随机生成项目 → Phase 2: 使用主 Skill 监控修复
```

**Phase 2 直接使用 `skill/SKILL.md` 中的监控和修复流程**，不重复编写。

---

## 重要：本地开发特殊说明

由于在 **agent-aware monorepo 内部测试**，与主 Skill 有以下不同：

### 1. 不需要安装包

主 Skill 要求：
```bash
npm install --save-dev @reskill/agent-aware-server @reskill/agent-aware
```

**本地测试跳过此步骤**，直接使用 `packages/` 中的代码。

### 2. 启动 Server 方式不同

主 Skill：
```bash
npx agent-aware-server start
```

**本地测试使用：**
```bash
# 先构建
pnpm build

# 启动 Server（指定项目路径）
USER_PROJECT_ROOT=$(pwd)/examples/[项目名] node packages/server/dist/cli.js start
```

---

# Phase 1: 随机生成前端项目

## 1.1 构建 packages

```bash
pnpm build
```

## 1.2 随机生成项目【Agent 自由发挥】

在 `examples/` 目录下，**随机生成一个 React + Vite 技术栈的前端项目**。

**要求：**
- 项目名称随机（如 `test-app-{timestamp}`）
- 技术栈：React + Vite
- 包含完整的项目结构（package.json、vite.config、src/main.jsx、src/App.jsx 等）
- 在 `package.json` 中添加本地依赖：`"@reskill/agent-aware": "link:../../packages/sdk"`
- 在入口文件中初始化 SDK：`initAgentAware({ debug: true })`
- **故意埋入 2-3 个问题**（dead_click、rage_click、runtime_error）
- **添加测试提示区域**，告诉用户哪些按钮会触发什么问题

**Agent 完全自由发挥，不限制具体内容和样式。**

## 1.3 测试提示区域【重要】

在页面中添加一个提示区域，包含：
- 哪些按钮/操作会触发问题
- 预期的问题类型
- Runtime Error 的触发条件和预期错误信息

---

# Phase 2: 使用主 Skill 监控修复

## 2.1 启动服务

```bash
# 1. 将 examples 加入 workspace（否则 pnpm install 无法解析 link: 依赖）
# 在 pnpm-workspace.yaml 中添加：
#   - 'examples/*'

# 2. 安装依赖
pnpm install --no-frozen-lockfile

# 3. 启动 Server（替换 [项目名] 为实际项目名）
USER_PROJECT_ROOT=$(pwd)/examples/[项目名] node packages/server/dist/cli.js start &

# 4. 启动前端开发服务器
pnpm --filter [项目名] dev &

# 5. 清空旧数据
curl -X DELETE http://localhost:4100/behaviors
curl -X DELETE http://localhost:4100/errors
```

**注意**：必须先将 `examples/*` 加入 `pnpm-workspace.yaml`，否则 `link:../../packages/sdk` 依赖无法正确解析。

## 2.2 提示用户预览

```
✅ 服务已启动！

请在 Cursor 中打开内置浏览器预览：
Cmd+Shift+P → "Simple Browser: Show" → 输入 http://localhost:5173

或点击终端链接 http://localhost:5173
```

## 2.3 按照主 Skill 执行监控

**从这里开始，完全按照 `skill/SKILL.md` 的流程执行。**

---

# Phase 3: 验证与报告

## 3.1 生成详细报告

```
📊 E2E 测试报告
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

📦 生成的应用: [应用主题]
📁 项目路径: examples/[项目名]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎯 预埋问题清单:

   1. [问题类型] - [按钮名称]
      原因: [描述]
      
   2. ...

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🔍 检测结果:

   Runtime Error: [详细错误信息和代码位置]
   Dead Click: [元素选择器和触发次数]
   Rage Click: [元素选择器和触发次数]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🔧 修复内容:

   1. [具体修复方案]
   2. ...

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

✅ 验证结果:

   | 指标 | 修复前 | 修复后 |
   |-----|-------|-------|
   | 挫折指数 | X | Y |
   | Runtime Error | N | 0 |
   | Rage Click | N | 0 |

   主 Skill 有效性: ✅ 验证通过

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

---

# Phase 4: 清理

```bash
# 1. 停止进程
pkill -f "agent-aware" || true
pkill -f "vite" || true

# 2. 删除测试项目
rm -rf examples/test-app-*

# 3. 恢复 pnpm-workspace.yaml（移除 examples/*）
# 将 pnpm-workspace.yaml 恢复为只包含 packages/*
```

**重要**：清理时务必恢复 `pnpm-workspace.yaml`，移除 `examples/*` 配置。

---

# 总结

| 阶段 | 内容 |
|-----|------|
| Phase 1 | Agent 随机生成 React+Vite 项目，埋入问题 |
| Phase 2 | 使用 `skill/SKILL.md` 监控修复 |
| Phase 3 | 生成详细报告 |
| Phase 4 | 清理 |

**核心价值：**
1. 测试 agent-aware 完整功能
2. 验证主 Skill 文档可行性
3. Agent 自由创作，更接近真实场景
